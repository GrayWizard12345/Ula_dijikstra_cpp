...
public:
    bool ifFoundReplaceElseInsert(Node &node){
        bool found = false;
...
struct SearchResult
{
    ...
    std::list<Node> path;
    double cost;
    int steps; //количетсво итераций алгоритма
    int createdNodes; //количество созданных вершин, OPEN.size + CLOSED.size
    double runtime; //время работы
    bool pathfound;
};
